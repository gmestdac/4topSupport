...
l1RPath = '/storage_mnt/storage/user/gmestdac/public_html/TTTT/YEAR/hpe-onlyPROC-norat-FINALVERpls/ee/SEL-l1NCF/'
l1WPath = '/storage_mnt/storage/user/gmestdac/public_html/TTTT/YEAR/hpe-onlyPROC-norat-FINALVERpls/ee/SEL-l1CF/'
l2RPath = '/storage_mnt/storage/user/gmestdac/public_html/TTTT/YEAR/hpe-onlyPROC-norat-FINALVERpls/ee/SEL-l2NCF/'
l2WPath = '/storage_mnt/storage/user/gmestdac/public_html/TTTT/YEAR/hpe-onlyPROC-norat-FINALVERpls/ee/SEL-l2CF/'
...
for ver in ['']:
  ...
  # for year in ['2016', '2017']:
    ...
    for plot in ['ptEtaNEWbinning']:
      ...
      for proc in ['DYpile', 'TTpile'] + ([i + p for i in ['DYpile'] for p in ['Pre', 'Post']] if year == '2016' else []):
      # for proc in ['TTpile'] + ([i + p for i in ['DYpile'] for p in ['Pre', 'Post']] if year == '2016' else []):
        ...
        for sel in ['onZ-nLep2-lepsPrompt']:
          try:
            ...
            l1R = sumHists(pickle.load(open(l1RPath.replace('YEAR', year).replace('PROC', proc).replace('SEL', sel).replace('VER', ver) + plot + '_l1.pkl', 'r'))[plot + '_l1'], [] , ['data'])
            l1W = sumHists(pickle.load(open(l1WPath.replace('YEAR', year).replace('PROC', proc).replace('SEL', sel).replace('VER', ver) + plot + '_l1.pkl', 'r'))[plot + '_l1'], [] , ['data'])
            l2R = sumHists(pickle.load(open(l2RPath.replace('YEAR', year).replace('PROC', proc).replace('SEL', sel).replace('VER', ver) + plot + '_l2.pkl', 'r'))[plot + '_l2'], [] , ['data'])
            l2W = sumHists(pickle.load(open(l2WPath.replace('YEAR', year).replace('PROC', proc).replace('SEL', sel).replace('VER', ver) + plot + '_l2.pkl', 'r'))[plot + '_l2'], [] , ['data'])
            ...
            ratio.SaveAs('chamidRatFinal' + '/chamid_' + year + '_' + proc + '_' + sel + '_' + plot + '.root')
            

            pickle.dump(ratio, file('chamidRatFinal' + '/chamid_' + year + '_' + proc + '_' + sel + '_' + plot + '.pkl', 'w'))
            ...
            ratio.GetZaxis().SetTitle("Charge misidentification rate   ")
            ratio.GetYaxis().SetTitleOffset(1.3-0.4)
            ratio.GetXaxis().SetTitleOffset(1.1-0.4)
            ratio.GetZaxis().SetTitleOffset(1.6-0.4)
            ...
            canv.SaveAs('chamidRatFinal' +'/chamid_' + year + '_' + proc + '_' + sel + '_' + plot + '.pdf')
            # canv.SaveAs('chamidRat' + ver +'/chamid_' + year + '_' + proc + '_' + sel + '_' + plot + '.png')
